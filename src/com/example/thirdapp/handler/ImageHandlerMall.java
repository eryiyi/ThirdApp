...
import com.example.thirdapp.fragment.MallFragment;

import java.lang.ref.WeakReference;

public class ImageHandlerMall extends Handler{
	/** 
     * 请求更新显示的View�? 
     */  
  ...
  //使用弱引用避免Handler泄露.这里的泛型参数可以不是Activity，也可以是Fragment�?  
    ...
    public ImageHandlerMall(WeakReference<MallFragment> wk){  
        weakReference = wk;  
    }  
    ...
    public void handleMessage(Message msg) {  
        ...
        MallFragment fragment = weakReference.get();  
        if (fragment==null){  
            //Activity已经回收，无�?再处理UI�?  
            return ;  
        }  
        //�?查消息队列并移除未发送的消息，这主要是避免在复杂环境下消息出现重复等问题�?  
        if (fragment.handlermall.hasMessages(MSG_UPDATE_IMAGE)){  
        	fragment.handlermall.removeMessages(MSG_UPDATE_IMAGE);  
        }  
        switch (msg.what) {  
...
//            //准备下次播放
//            fragment.handlermall.sendEmptyMessageDelayed(MSG_UPDATE_IMAGE, MSG_DELAY);
            break;  
