...
import com.example.thirdapp.fragment.MainPageTitleFragment;
...
public class Xq extends BaseActivity implements OnClickListener{
	ListView villagelv;
	...
	protected void onCreate(Bundle arg0) {
		...
		adapter = new ItemXiaoquAdapter(lists, Xq.this);
		lstv.setAdapter(adapter);
		lstv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
				Community community = lists.get(i);
				Intent intent = new Intent(Xq.this, MainPageTitleFragment.class);
		...
		if(!StringUtil.isNullOrEmpty(String.valueOf(ThirdApplication.lat))){
			...
			progressDialog = new CustomProgressDialog(Xq.this , "", R.anim.frame_paopao);
						...
						if (StringUtil.isJson(s)) {
							try {
								...
								if(Integer.parseInt(code) == 200){
									...
									Toast.makeText(Xq.this, jo.getString("msg"), Toast.LENGTH_SHORT).show();
								}
							} catch (JSONException e) {
								e.printStackTrace();
							}
						} else {
							Toast.makeText(Xq.this, R.string.get_data_error, Toast.LENGTH_SHORT).show();
						}
